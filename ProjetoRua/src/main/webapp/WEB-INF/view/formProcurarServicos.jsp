...
<%@ page
	import="br.com.tcc.projetorua.model.Localizacao,br.com.tcc.projetorua.model.Categoria,br.com.tcc.projetorua.security.FormatUtf8, br.com.tcc.projetorua.model.Recorrencia, br.com.tcc.projetorua.model.Servico"%>
	...
	<main id="principal">
		...
		<div class="container ">
			<div class="d-flex col-md-10 mx-auto">
				...
				<form class="my-2 row g-3" method="post"
					action="/ProjetoRua/controller">


					<label class="col-md-2 col-sm-4 form-label ">Categoria</label> <label
						class="col-md-2 col-sm-4 form-label ">Seu CEP</label> <label
						class="col-md-8 col-sm-4 form-label ">Distância</label>
					...
					<div class="col-md-2 col-sm-4">
						...
						<input class="form-control" name="distancia" type="text" value=""
							size="50">
					</div>
					...
					<div class="col-md-3 col-sm-12">
						<div class="row">
							...
							<div class="col-md-5 col-sm-4">
								<a href="/ProjetoRua/controller?call=ProcurarServicosForm"
									class="btn text-center  btn-custom btn-outline-light">Limpar
								</a>
							</div>
							<div class="col-md-5 col-sm-4">
								<input class="text-center btn  btn-custom btn-outline-light"
						...
						<input type="hidden" name="call" value="ProcurarServicos">
		...
		<ul class="list-unstyled  ">
			...
			<c:choose>
				...
				<c:otherwise>
					...
					<c:choose>
						<c:when test="${empty listaLocalizacao}">
								...
								<c:forEach var="i" begin="0" end="${fn:length(servicos) - 1}">
									...
									<li>
										<div id="servicos-card" class=" container ">
											...
											<div class="col-md-10 mx-auto card text-white bg-dark  p-1">
												<div class="row">
													...
													<div class="row">
														...
														<div class="col-md-12">
															<span class="fw-bold">
																${formatUtf8.getUtf8(servicos[i].getPrestador().getNome() )}</span>
														</div>
													...
													<div class="row">
														...
														<div class="col-md-12" style="text-align: justify;">${formatUtf8.getUtf8( servicos[i].getDescricao()) }</div>
													...
													<div class="row">
														...
														<c:if test="${servicos[i].getRecorrencia().getSegunda()}">


															<div class="col-md-12">Segunda-feira:
																${servicos[i].getRecorrencia().getHorarioInicio() } -
																${servicos[i].getRecorrencia().getHorarioTermino() }</div>

														</c:if>
														<c:if test="${servicos[i].getRecorrencia().getTerca()}">

															<div class="col-md-12">Terça-feira:
																${servicos[i].getRecorrencia().getHorarioInicio() } -
																${servicos[i].getRecorrencia().getHorarioTermino() }</div>
														</c:if>
														<c:if test="${servicos[i].getRecorrencia().getQuarta()}">

															<div class="col-md-12">Quarta-feira:
																${servicos[i].getRecorrencia().getHorarioInicio() } -
																${servicos[i].getRecorrencia().getHorarioTermino() }</div>
														</c:if>
														<c:if test="${servicos[i].getRecorrencia().getQuinta()}">

															<div class="col-md-12">Quinta-feira:
																${servicos[i].getRecorrencia().getHorarioInicio() } -
																${servicos[i].getRecorrencia().getHorarioTermino() }</div>
														</c:if>
														<c:if test="${servicos[i].getRecorrencia().getSexta()}">

															<div class="col-md-12">Sexta-feira:
																${servicos[i].getRecorrencia().getHorarioInicio() } -
																${servicos[i].getRecorrencia().getHorarioTermino() }</div>

														</c:if>
														<c:if test="${servicos[i].getRecorrencia().getSabado()}">

															<div class="col-md-12">Sábado:
																${servicos[i].getRecorrencia().getHorarioInicio() } -
																${servicos[i].getRecorrencia().getHorarioTermino() }</div>
														</c:if>
														<c:if test="${servicos[i].getRecorrencia().getDomingo()}">

															<div class="col-md-12">Domingo:
																${servicos[i].getRecorrencia().getHorarioInicio() } -
																${servicos[i].getRecorrencia().getHorarioTermino() }</div>
														</c:if>
								...
								<c:forEach var="i" begin="0" end="${fn:length(servicos) - 1}">
									<li>
										<div id="servicos-card" class=" container ">
											...
											<div
												class="col-md-10 col-sm-12 mx-auto card text-white bg-dark mb-4 p-1">
												<div class="row">
													...
													<div class="row">
														...
														<div class="col-md-12 col-sm-12">
															<span class="fw-bold">${formatUtf8.getUtf8(servicos[i].getPrestador().getNome() )}</span>
														</div>
													...
													<!-- 														<div class="col-md-12"><span class="fw-bold">Logradouro:</span>    -->
													<%-- 															${formatUtf8.getUtf8(servicos[i].getEndereco().getLogradouro() )}, --%>
													<%-- 															${formatUtf8.getUtf8(servicos[i].getEndereco().getNumero()) } --%>
													<%-- 															- ${ formatUtf8.getUtf8(servicos[i].getEndereco().getBairro())}, --%>
													<%-- 															${ formatUtf8.getUtf8(servicos[i].getEndereco().getCidade())} --%>
													<%-- 															- ${ formatUtf8.getUtf8(servicos[i].getEndereco().getEstado())}, --%>
													<%-- 															${servicos[i].getEndereco().getCep() }</div> --%>
													...
													<div class="row">
														...
														<div class="col-md-12">
															<span class="fw-bold">Origem:</span>${formatUtf8.getUtf8( listaLocalizacao[i].getOrigem())}
														</div>
														...
														<%-- 											<div class="col-md-12"><span class="fw-bold"> Destino:</span> 	${listaLocalizacao[i].getDestino()}</div> --%>

														<div class="col-md-12">
															<span class="fw-bold"> Distancia:</span>
															${listaLocalizacao[i].obterDescricaoDistancia()}
														</div>

														<div class="col-md-12">
															<span class="fw-bold"> Tempo:</span>
															${listaLocalizacao[i].obterDescricaoTempo()}
														</div>
													</div>
													<iframe id="map" class="mt-2"
														src="${listaLocalizacao[i].getGoogleMapsURL()}"
														width="820" height="520" frameborder="0" marginwidth="0"
